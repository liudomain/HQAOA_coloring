...
from multilevel_common import (
    ...
    visualize_graph,
    handle_exception,
    smart_divide_graph_with_qubit_constraint,  # 新增：智能子图划分函数
)
# 从三个专门模块导入各自特有的函数
...
def main_adapt(graphs, dataset, graph_index, seed):
    ...
    for idx, graph in enumerate(graphs, start=1):
        ...
        try:
            ...
            # 2 划分 (使用智能划分方法，自动满足量子比特约束)
            divide_start = time.time()
            # 使用智能划分：自动根据20比特约束调整子图大小
            # max_k=20 (原版配置), max_qubits=20
            subgraphs, sub_maps, divide_info = smart_divide_graph_with_qubit_constraint(
                graph,
                max_qubits=20,  # MindQuantum最大量子比特限制
                max_k_per_subgraph=algo_params["max_k"],  # 20
                Q=algo_params["Q"]
            )
            print(f"智能划分完成: {len(subgraphs)} 个子图")
            print(f"  最大节点数/子图: {divide_info['max_nodes_per_subgraph']}")
            print(f"  比特数/节点: {divide_info['bits_per_node']}")
            avg_qubits = divide_info.get('avg_qubits', 0)
            print(f"  平均比特数: {avg_qubits:.1f}")
...
def parse_test_args():
    ...
    parser.add_argument('--adapt-standard', action='store_true', help='Test Adapt-QAOA and Standard-QAOA (no noise)')
...
def main_test():
    ...
    # 处理 --adapt-standard 快捷选项
    if args.adapt_standard:
        args.adapt = True
        args.standard = True
        args.adapt_noise = False

    # Validate that at least one algorithm is selected
    if not any([args.adapt, args.standard, args.adapt_noise]):
        print("⚠️ Must select at least one algorithm (--adapt/--standard/--adapt-noise/--adapt-standard)")
        ...
        print("  python Main_Multilevel_qaoa.py --adapt-standard")
        return
    ...
    if args.adapt_standard:
        print(f"  - Quick Mode: Adapt + Standard (no noise)")
    if args.adapt_noise:
...
# # ============ 使用示例 ============
# # Main_Multilevel_qaoa_smart_divide.py 使用示例（智能划分版本）
# # 该文件已被简化，功能已合并到 Main_Multilevel_qaoa.py
# # 请使用 run_experiments.py 进行实验
