...
本模块实现基于标准QAOA(Standard QAOA)的多层次图着色算法。
标准QAOA使用固定的混合算子（通常是X门），每层的混合算子相同。

主要包含：
1. solve_k_coloring_standard: 使用标准QAOA求解k着色问题
2. sequential_process_subgraphs_standard: 顺序处理子图着色
3. iterative_optimization_standard: 迭代优化全局着色方案
...
与自适应QAOA的区别：
- 标准QAOA：每层混合算子固定，参数优化简单
- 自适应QAOA：每层通过梯度选择最优混合算子，可能获得更好性能但计算开销更大
"""
...
from multilevel_common import (
    ...
    build_hamiltonian,
    qaoa_ansatz_standard as qaoa_ansatz_standard,
...
def solve_k_coloring_standard(graph, k, p=1, num_steps=1000, vertex_colors=None,
    ...
    try:
        ...
        # 构建标准QAOA量子线路和哈密顿量
        # -----------------------------------------------------------------
        circ = qaoa_ansatz_standard(graph, k, p, vertex_colors, nodes_to_recolor, penalty)
...
def sequential_process_subgraphs_standard(
    subgraphs,
    sub_mappings,
    dataset_name,
    graph_id,
    ...
    # os.makedirs(os.path.join(os.path.join(BASE_DIR, "results"), dataset_name), exist_ok=True)  # 注释掉 results/BeijingSubway 文件夹生成

    # 创建子图处理详细日志
    detailed_subgraph_log = os.path.join(LOGS_DIR, f"{dataset_name}_graph_{graph_id}_detailed_log.csv")
    if not os.path.exists(detailed_subgraph_log):
        with open(detailed_subgraph_log, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([
                "subgraph_index", "graph_type", "nodes", "edges",
                "min_k", "conflicts", "status", "processing_time",
                "retry_attempts", "success_attempt", "conflict_history"
            ])
    ...
    if isolated_subgraphs:
        ...
        for i, sub, mapping in isolated_subgraphs:
            ...
            try:
                ...
                # 记录详细日志
                with open(detailed_subgraph_log, "a", newline="", encoding="utf-8") as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        i + 1, "isolated", len(sub.nodes), len(sub.edges),
                        1, 0, 'success (isolated)', round(processing_time, 4),
                        1, 1, "[0]"
                    ])
                ...
                with open(detailed_subgraph_log, "a", newline="", encoding="utf-8") as f:
                    ...
                    writer.writerow([
                        i + 1, "isolated", len(sub.nodes), len(sub.edges),
                        1, float('inf'), f'failed: {str(e)}', round(processing_time, 4),
                        1, None, "[]"
                    ])
    ...
    for i, (sub, mapping) in enumerate(zip(subgraphs, sub_mappings)):
        ...
        if len(sub.nodes) == 0:
            ...
            with open(detailed_subgraph_log, "a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow([
                    i + 1, "empty", 0, 0, 1, 0, 'skipped', round(processing_time, 4),
                    0, None, "[]"
                ])
            continue
        ...
        try:
            ...
            with open(detailed_subgraph_log, "a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow([
                    i + 1, "unknown", len(sub.nodes), len(sub.edges),
                    1, float('inf'), f'failed (mapping error): {str(e)}', round(processing_time, 4),
                    0, None, "[]"
                ])
            continue
        ...
        if len(new_sub.nodes) == 0:
            ...
            with open(detailed_subgraph_log, "a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow([
                    i + 1, "empty_after_mapping", 0, 0, 1, 0, 'skipped', round(processing_time, 4),
                    0, None, "[]"
                ])
            continue
        ...
        if is_cycle_graph(new_sub):
            ...
            try:
                ...
                with open(detailed_subgraph_log, "a", newline="", encoding="utf-8") as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        i + 1, f"cycle_{cycle_type}", len(sub.nodes), len(sub.edges),
                        cycle_k, conflict_count, 'success (cycle graph)', round(processing_time, 4),
                        1, 1, "[0]"
                    ])
                ...
                with open(detailed_subgraph_log, "a", newline="", encoding="utf-8") as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        i + 1, f"cycle_{cycle_type}_fallback", len(sub.nodes), len(sub.edges),
                        required_k, conflict_count, 'success (cycle fallback greedy)', round(processing_time, 4),
                        1, 1 if conflict_count == 0 else None, str([conflict_count])
                    ])
        ...
        if sub_signature in signature_cache:
            ...
            with open(detailed_subgraph_log, "a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow([
                    i + 1, "ordinary_cached", len(sub.nodes), len(sub.edges),
                    cached_k, conflict_count, 'success (cached)', round(processing_time, 4),
                    1, 1 if conflict_count == 0 else None, str([conflict_count])
                ])
        ...
        for k in k_candidates:
            ...
            logger.info(f"\n尝试k={k}着色...")
            try:
                qaoa_result = solve_k_coloring_standard(
                    graph=new_sub,
                    k=k,
                    p=p,
                    num_steps=num_steps,
                ...
                logger.info(f"k={k} 完成：收敛参数={conv_param:.1f}，冲突数={current_conflict}")
        ...
        with open(detailed_subgraph_log, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([
                i + 1, "ordinary", len(sub.nodes), len(sub.edges),
                final_k, final_conflict, sub_result['status'], round(processing_time, 4),
                retry_info['total_attempts'], retry_info['success_attempt'],
                str(retry_info['conflict_history'])
            ])
...
def iterative_optimization_standard(
    graph,
    subgraphs,
    sub_mappings,
    ...
    注意：此函数不直接使用QAOA，仅用于标准QAOA版本的整体流程兼容
    ...
    for it in range(max_iter):
        ...
        for u, v in conflict_edges:
            nodes_to_fix.add(u)
            nodes_to_fix.add(v)
        nodes_to_fix = list(nodes_to_fix)
        ...
        # 3.6.2 局部微调（高冲突节点优先）
        # -------------------------------------------------------------
        # 初始化节点分数字典（确保包含所有冲突节点）
        node_score = {n: 0 for n in nodes_to_fix}

        # 统计冲突次数（安全处理，避免KeyError）
        for u, v in find_conflict_edges(curr_coloring, graph):
            if u in node_score:  # 仅更新已记录的节点
                node_score[u] += 1
            if v in node_score:  # 仅更新已记录的节点
                node_score[v] += 1
        ...
        # 按分数排序，优先处理高冲突节点
        for node in sorted(nodes_to_fix, key=lambda x: node_score[x], reverse=True):
