...
def main_adapt(graphs, dataset, graph_index, seed):
    ...
    for idx, graph in enumerate(graphs, start=1):
        ...
        try:
            ...
            # 2 划分
            n_sub = max(2, int(np.sqrt(n_nodes)))
            subgraphs, sub_maps = divide_graph(graph, n_sub, Q=algo_params["Q"])
            print(f"Adapt-QAOA 划分完成：{len(subgraphs)} 个子图")
...
def main_standard(graphs, dataset, graph_index, seed):
    ...
    for index, graph in enumerate(graphs, start=1):
        ...
        try:
            ...
            # 2. Divide subgraphs (reuse from adapt's divide_graph)
            num_subgraphs = max(2, int(np.sqrt(num_nodes)))
            subgraphs, sub_mappings = divide_graph(graph, num_subgraphs, Q=algorithm_params["Q"])
...
def main_adapt_noise(graphs, dataset, graph_index, seed, depolarizing_prob=0.01):
    ...
    for idx, graph in enumerate(graphs, start=1):
        ...
        try:
            ...
            # 2. Divide subgraphs
            n_sub = max(2, int(np.sqrt(n_nodes)))  # Number of subgraphs
            subgraphs, sub_maps = divide_graph(graph, n_sub, Q=algo_params["Q"])
            print(f"Adapt-noise-QAOA division completed: {len(subgraphs)} subgraphs")
...
def main():
    ...
    seed = 10
    # Extract dataset name from the loaded graph files
    if graphs and hasattr(graphs[0], 'file_name'):
        # Extract dataset name from graph file path
        graph_path = graphs[0].file_name
        dataset_name = os.path.basename(os.path.dirname(graph_path))
        dataset = dataset_name if dataset_name else 'graph_dataset'
    else:
